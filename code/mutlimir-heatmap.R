...
names(multimirRes) <- miRNAnames
multimirRes <- multimirRes[c(2, 3, 5, 7, 1, 4, 6)]

targets <- do.call(rbind, multimirRes)
...
countFpkm <- distinct(countFpkm, name, .keep_all = TRUE)
countFpkmRownmeans <- rowMeans(countFpkm[,c(8:40)], na.rm = TRUE)
countFpkm$rowMeans <- countFpkmRownmeans
countFpkm <- filter(countFpkm, rowMeans > 2)
